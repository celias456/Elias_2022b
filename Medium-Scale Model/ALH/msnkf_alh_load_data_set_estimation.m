function [data,Sigma_hat,c,first_observation,Sigma_u_sd,theta,prior_information,T] = msnkf_alh_load_data_set_estimation(data_set_identifier)
...
if data_set_identifier == 1 %2001
    ...
    Sigma_hat = load('msnkf_alh_sigma_hat_2001.csv'); 
    
    %Load the parameter information
    parameter_info = load('msnkf_alh_parameters_2001.csv');
...
elseif data_set_identifier == 2 %2007
    ...
    %Covariance matrix used for the variance of the proposal distribution in the MH algorithm (obtained from Dynare)
    Sigma_hat = load('msnkf_alh_sigma_hat_2007.csv'); 
    
    %Load the parameter information
    parameter_info = load('msnkf_alh_parameters_2007.csv');
    ...
    %Covariance matrix used for the variance of the proposal distribution in the MH algorithm (obtained from Dynare)
    Sigma_hat = load('msnkf_alh_sigma_hat_2020.csv'); 
    
    %Load the parameter information
    parameter_info = load('msnkf_alh_parameters_2020.csv');
