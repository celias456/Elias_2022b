...
[mh_theta,mh_theta_A,mh_theta_B,mh_theta_R_A,mh_theta_R_B,mh_theta_s,mh_theta_P,data_forecast_sample,T,H,TpH,Sigma_u_sd] = ssnkf_al_load_data_set_forecast(data_set_identifier);
...
for count_m2 = 1:m2
    ...
    [G_1,G_2,G_3,G_4,H_1,H_2,invertible,Sigma_v] = ssnkf_al_condensed_form(number_endogenous_variables,number_exogenous_variables,RE_Gamma_0,RE_Gamma_1,theta);
    ...
    for count_m1 = 1:m1
        ...
        for j = 3:TpH   
            ...
            [A(:,:,j),B(:,:,j),R_A(:,:,j),R_B(:,:,j)] = ssnkf_al_update_beliefs(number_endogenous_variables,number_exogenous_variables,theta,s_last_two_periods,A(:,:,j-1),B(:,:,j-1),R_A(:,:,j-1),R_B(:,:,j-1));
